...
for depth in sorted(levels):
    build_order.extend(sorted(levels[depth]))

report = {
    "module": "FORGE-BUILD-0003",
    "status": "PASS",
    "runtime_mode": "SHADOW_ONLY_READ_ONLY",
    "generated_at": datetime.now(UTC).isoformat(),
    "target_module": root,
    "build_order": build_order,
    "levels": {str(k): sorted(v) for k, v in sorted(levels.items())},
    "build_steps": len(build_order)
}
...
OUT = ROOT / "runtime/build/build_order_plan.json"
OUT.write_text(json.dumps(report, indent=2))

print("=" * 60)
print("FORGE-BUILD-0003")
print("Build Order Planner Engine")
print("=" * 60)
print("Target      :", root)
print("Build Steps :", len(build_order))
