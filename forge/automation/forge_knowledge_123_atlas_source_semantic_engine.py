...
semantic = {
    "module": "FORGE-KNOWLEDGE-123",
    "status": "ATLAS_SOURCE_SEMANTIC_ENGINE_READY",
    "source_engine": "FORGE-KNOWLEDGE-122_ATLAS_SOURCE_MEMORY_ENGINE",
...
(RUNTIME / "source_semantic_123.json").write_text(
    json.dumps(semantic, indent=2)
)

(RUNTIME / "source_semantic_123_hash.json").write_text(
    json.dumps({
        "module": "FORGE-KNOWLEDGE-123",
        "hash": hash_value
    }, indent=2)
...
with open(RUNTIME / "source_semantic_123_ledger.jsonl", "a") as f:
    f.write(json.dumps(semantic) + "\n")

print("FORGE-KNOWLEDGE-123 ATLAS SOURCE SEMANTIC ENGINE READY")
