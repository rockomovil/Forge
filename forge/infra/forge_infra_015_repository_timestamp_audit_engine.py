#!/usr/bin/env python3

import hashlib
import json
from datetime import datetime, timezone
from pathlib import Path

MODULE = "FORGE-INFRA-015"
STATUS = "FORGE_REPOSITORY_TIMESTAMP_AUDIT_READY"

ROOT = Path(__file__).resolve().parents[2]

files = 0
oldest_mtime = None
newest_mtime = None

for path in ROOT.rglob("*"):
    if ".git" in path.parts or not path.is_file():
        continue

    try:
        mtime = path.stat().st_mtime
    except OSError:
        continue

    files += 1

    if oldest_mtime is None or mtime < oldest_mtime:
        oldest_mtime = mtime

    if newest_mtime is None or mtime > newest_mtime:
        newest_mtime = mtime

payload = {
    "module": MODULE,
    "status": STATUS,
    "files_audited": files,
    "oldest_timestamp": datetime.fromtimestamp(
        oldest_mtime, tz=timezone.utc
    ).isoformat(),
    "newest_timestamp": datetime.fromtimestamp(
        newest_mtime, tz=timezone.utc
    ).isoformat(),
    "runtime_mode": "SHADOW_ONLY_READ_ONLY",
    "timestamp": datetime.now(timezone.utc).isoformat(),
}

serialized = json.dumps(payload, sort_keys=True, indent=2)
integrity_hash = hashlib.sha256(serialized.encode()).hexdigest()
payload["integrity_hash"] = integrity_hash

runtime_dir = ROOT / "runtime" / "infra"
runtime_dir.mkdir(parents=True, exist_ok=True)

(runtime_dir / "forge_repository_timestamp_015.json").write_text(
    json.dumps(payload, indent=2),
    encoding="utf-8",
)

hash_payload = {
    "module": MODULE,
    "hash": hashlib.sha256(
        json.dumps(payload, sort_keys=True).encode()
    ).hexdigest(),
}

(runtime_dir / "forge_repository_timestamp_015_hash.json").write_text(
    json.dumps(hash_payload, indent=2),
    encoding="utf-8",
)

with (runtime_dir / "forge_repository_timestamp_015_ledger.jsonl").open(
    "a",
    encoding="utf-8",
) as ledger:
    ledger.write(json.dumps(payload) + "\n")

print(MODULE)
print(STATUS)
print(f"files_audited = {files}")
print(f"oldest_timestamp = {payload['oldest_timestamp']}")
print(f"newest_timestamp = {payload['newest_timestamp']}")
print(f"integrity_hash = {integrity_hash}")
print(f"hash = {hash_payload['hash']}")
print("runtime_mode = SHADOW_ONLY_READ_ONLY")
print(f"{MODULE} VERIFIED")
